        ...
        <React.Fragment>
            <Card>
                <CardHeader className="d-flex align-items-center justify-content-between flex-wrap">
                    ...
                    <Button as={Link} className="btnaccept d-inline-flex align-items-center justify-content-center" variant={null}><Icon icon="icon-park-outline:check-one"/>Accept</Button>
                    <Button as={Link} className="btnreject d-inline-flex align-items-center justify-content-center" variant={null}><Icon icon="ion:close-circle-outline"/>Reject</Button>
                    <Button as={Link} to="/rfqs" className="d-inline-flex align-items-center justify-content-center">Back To RFQ's</Button>
                    </div>
                ...
                <CardBody>
                    <Row>
                        ...
                        <Col md={6} lg={4} xl={3} className="mb-3">
                            <div className="orders-card">
                                <div className="d-flex align-items-center mb-3"><label>Order Date: </label> <span>6-9-24</span></div>
                                <div className="d-flex align-items-center mb-3"><label>Due Date: </label>   <span>-</span></div>
                                <div className="d-flex align-items-center mb-3"><label>Order Amount: </label>  <span>-</span></div>
                                <div className="d-flex align-items-center"><label>Status: </label>  <span>-</span></div>
                            </div>
                        </Col>
                    ...
                    <div className="orders-shipping d-flex align-items-center justify-content-between flex-wrap">
                        <div className="d-inline-flex align-items-center gap-2 my-1">
                            <b>In this order:</b>
                            <span className="badgestatus" style={{ backgroundColor: "#e11f26" }}>CS0120</span>
                            <span className="badgestatus" style={{ backgroundColor: "#4F8CCA" }}>A50120</span>
                        </div>
                        <div className="d-inline-flex align-items-center gap-2 my-1">
                            <b>Shipping:</b>
                            <span className="badgestatus" style={{ backgroundColor: "#CB6CE6" }}>UPS</span>
                        </div>
                    </div>
                    <div className="orders-detail mt-3">
                        {workOrders.map((wo, index) => (
                            <div key={index} className="list-main  d-inline-flex justify-content-between w-100">
                                <div className="list-left d-inline-flex">
                                    ...
                                    <div className="list-content">
                                        <h2>{wo.name} <Icon icon="material-symbols-light:download-sharp" /></h2>
                                        <div className="list-qty d-flex align-items-center gap-3 mb-3">
                                            <span className="qty"><strong>QTY:</strong> {wo.qty}</span>
                                            <span className="price-total">${wo.price}/ea.</span>
                                            <span className="price-total">${wo.total}/total</span>
                                        </div>
                                        <Link className="btnnote" onClick={handleShow}>View Notes</Link>
                                    </div>
                                </div>

                                <div className="list-checkboxes  d-inline-flex gap-3">
                                    {wo.labels.map(label => (
                                        <div className="custom-checkbox-container text-center" key={label}>
                                            <label className="custom-label" htmlFor={`${label}${wo.id}`} style={getLabelsColor(wo.materials)}>{label}</label>
