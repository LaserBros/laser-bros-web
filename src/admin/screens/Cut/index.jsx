                    ...
                    <Form>
                        <Row className="px-2 gx-3">
                            ...
                            <Col lg={3} xxl={6} className="text-lg-end">
                                <Button variant={null} className="btn-outline-primary min-width-147">
                                    Archive
                                </Button>
                            </Col>
                        </Row>
                            ...
                            <tbody>
                                {data.map(row => (
                                    <React.Fragment key={row.id}>
                                        <tr className={expandedRow === row.id ? 'expanded-row' : ''}>
                                            <td className='text-nowrap'><b>Material:</b><span className="badgestatus" style={getMaterialColor(row.material)}>{row.material}</span></td>
                                            <td className='text-nowrap'><b>Number Of Parts:</b>{row.parts}</td>
                                            <td className='text-nowrap'><b>Due:</b>{row.due}</td>
                                            <td className="text-end">
                                                <div className="d-inline-flex align-items-center">
                                                    ...
                                                    >
                                                        {expandedRow === row.id ? <Icon icon="teenyicons:minimise-alt-outline" /> : <Icon icon="teenyicons:expand-alt-solid" />}
                                                    </Button>
                                                    <Form.Check
                                                        type="checkbox"
                                            ...
                                            <tr>
                                                <td colSpan="4" style={{ borderRadius: 12 }}>
                                                    <Row className="expanded-top align-items-center mb-3">
                                                        <Col xs={5}>
                                                            <div className="d-inline-flex align-items-center checkbox-top gap-3"> {row.checkboxes.map(option => (
                                                                <Form.Check
                                                                    key={option}
                                                                    type="checkbox"
                                                                    id={`${option}1`}
                                                            ...
                                                            <div className="list-checkboxes  d-inline-flex gap-3">
                                                                {wo.labels.map((label, index) => (
                                                                    <div className="custom-checkbox-container text-center" key={label}>
                                                                        <label key={`${row.id}-${label}-${index}`} className="custom-label" htmlFor={`${label}${wo.id}`}  style={getMaterialColor(row.material)}>{label}</label>
